...
async fn main() -> Result<()> {
    ...
    for (lat, lon, description) in test_coordinates {
        println!("\n📍 Testing location detection for {}: {:.4}, {:.4}", description, lat, lon);
        
        match location_service.get_location_from_coordinates(lat, lon).await {
            Ok(location_data) => {
                println!("   ✅ Successfully detected: {}", location_data.display_name);
                ...
                println!("   📊 Data completeness: City: {}, Region: {}, Country: {}", 
                    if has_city { "✅" } else { "❌" },
                    if has_region { "✅" } else { "❌" },
                    if has_country { "✅" } else { "❌" }
                );
